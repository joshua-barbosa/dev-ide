...
export async function comprimir(
  ...
  // `ArrayBufferView` no lugar de `BlobPart`: os dois `Blob` — o do navegador e
  // o do Node — aceitam essa forma, e `BlobPart` é tipo do DOM, que o HOST da
  // extensão não compila. O host monta o zip pela MESMA função; duas
  // implementações do formato divergiriam num caso de canto, e o arquivo só
  // quebraria na mão de quem tentasse abrir.
  // **Uma cópia num buffer próprio**, e não um `as BlobPart`: aquele é tipo do
  // DOM, e o HOST da extensão compila sem DOM. O host monta o zip pela MESMA
  // função — duas implementações do formato divergiriam num caso de canto, e o
  // arquivo só quebraria na mão de quem tentasse abrir.
  const copia = new Uint8Array(dados.length);
  copia.set(dados);
  const fluxo = new Blob([copia]).stream().pipeThrough(

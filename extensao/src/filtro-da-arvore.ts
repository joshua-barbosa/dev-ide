...
export function interpretarTamanho(texto: string): number | null {
  ...
  // `m[1]`/`m[2]` existem porque o padrão casou — mas o compilador da EXTENSÃO
  // é mais severo (`noUncheckedIndexedAccess`) e não sabe disso. Escrever a
  // guarda é mais barato que afrouxar a regra do host, que protege código que
  // roda dentro do editor dele.
  const unidade = m[2] ?? '';
  const numero = m[1] ?? '';
  const fator = UNIDADES[unidade];
  if (fator === undefined) return null;
  const valor = Number(numero) * fator;

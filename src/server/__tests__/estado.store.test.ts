...
test('sem arquivo, a IDE não tem pasta aberta', () => {
  comStore((store, arquivo) => {
    assert.deepEqual(store.ler(), { pastaAtual: null, recentes: [] });
...
test('abrir persiste e sobrevive a reler', () => {
  comStore((store, arquivo) => {
    ...
    assert.equal(new EstadoStore(arquivo).ler().pastaAtual, '/casa/projeto');
  });
});
...
test('o histórico atravessa fechar', () => {
  comStore((store) => {
    ...
    assert.equal(depois.pastaAtual, null);

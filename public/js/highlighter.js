...
(function () {
  ...
  function tokenize(code, lang) {
    const def = window.LANGUAGES[lang] || window.LANGUAGES.plain;
    const tokens = [];
    let pos = 0;
  ...
  function contem(set, word, def) {
    return set.has(word) || (def.classify === 'sql' && set.has(word.toLowerCase()));
  }

  function classifyIdentifiers(tokens, def) {
    let declContext = null; // 'class' | 'const' | 'variable' | 'function' | null
  ...
  window.Highlighter = { tokenize, highlight };
})();
